 ...
 * <remarks>
 * <para>
 * The manager consumes normalized inputs via
 * <see cref="handleInput(Player, DreamPressedKeys, Instant)"/> and
 * matches them against active <see cref="IDreamKeyPatternSpec"/> instances
 ...
 * Features:
 * <ul>
 *   <li>Supports both <c>InOrder</c> sequences and <c>AllAtOnce</c> multi-key activations.</li>
 *   <li>Handles timing windows, chord spreads, per-step and global conditions, and per-pattern cooldowns.</li>
 *   <li>Patterns may be configured as <c>firstTimeOnly</c> (one-shot) or reusable.</li>
 *   <li>On success or failure, one-shot patterns are removed, while reusable patterns are re-armed.</li>
 *   <li>Thread-safe: uses concurrent collections for per-player attempt tracking.</li>
 * </ul>
 * </para>
 *
 * <para>
 * Typical usage is to drive inputs from the Bukkit main thread.
 * If called from other threads, ensure your listeners are thread-safe.
 * </para>
 * </remarks>
 */
public final class DreamKeyManager implements IDreamKeyManager {
    ...
    /** <summary>Create a manager using {@link Instant#now()} as the time source.</summary> */
    ...
    /** {@inheritDoc} */
    @Override
    public void handleInput(Player player, DreamPressedKeys key, Instant at) {
        ...
        for (var attempt : snapshot) {
            ...
            if (cdLeft != null && !cdLeft.isZero() && !cdLeft.isNegative()) {
    ...
    /** <summary>Validates a single hand against an {@link IDreamHandCondition}.</summary> */
    ...
    /**
     * <summary>
     * Returns true if {@code buffer} contains all keys from the step’s chord within its configured spread window.
     * </summary>
     */
    ...
    private void stepCompleted(Attempt a, Instant at) {
        a.lastStepTime = at;
        a.progressIndex++;
        a.listener.PartialComplete(a.playerId, a.progressIndex - 1);
    ...
    private void complete(Attempt a) {
        try {
            a.listener.ActionComplete(a.playerId);
    ...
    private void fail(Attempt a) {
        try {
            a.listener.FailedAction(a.playerId);
        } finally {
            if (a.pattern.firstTimeOnly()) {
                var list = attempts.get(a.playerId);
                if (list != null) list.remove(a);
    ...
    /**
     * <summary>
     * Returns true if the attempt’s pattern currently has cooldown remaining.
     * </summary>
     * <remarks>
     * Uses a safe check compatible with standard JDKs:
     * remaining &gt; 0 ⇔ !isZero() &amp;&amp; !isNegative()
     * </remarks>
     */
    private boolean isOnCooldown(Attempt a) {
        Duration left = cooldowns.remaining(new CooldownKey(a.playerId, a.pattern));
        return left != null && !left.isZero() && !left.isNegative();
    }
